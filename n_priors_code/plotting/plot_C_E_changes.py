...
for i, key in enumerate(par_gaps.keys()):
    ...
    for x in xrange(0,4):
        ...
        plot2 = plt.semilogx(dats[:, 0, 0], (dats[:, 2, 4 * i + x+1]-dats[:, 2, 0])/dats[:, 2,0], linewidth=1,rasterized=True,label=r'${} = {}$'.format(label[key],values[key][x]))
    ...
    plt.savefig('../data/{}/run{}/output/CL_E_{}.pdf'.format(data_type,run_idx,str(key)), dpi=400, papertype='Letter',

'''
Plot of 1 ellispse (2 parameters) for different probes CMB TT EE etc.
...
colors = ['r', 'g', 'b', 'y']
for i,probes in enumerate(['CMB', 'joint']):
    ...
    fisher_mat = np.loadtxt(
        base_dir + 'data/{}/run{}/output/fisher_mat_{}_lmin={}_lmax={}_ndet={}_fsky={}.txt'.format(data_type, str(run_idx), probes, lmin, lmax, N_det, fsky))
    ...
    # \===========================
    def integrand(z, omnu, H_0):
        omega_m = (fid['omch2'] + fid['ombh2'] + omnu) / H_0 ** 2
        # print omnuh2
        # print omnu,H_0
        return 1 / (H_0 * np.sqrt(omega_m * (1. + z) ** 3 + (1 - omega_m) ))

    H_0 = fid['hubble']
    omnuh2 = fid['omnuh2']
    R = quad(integrand, 0, 1100, args=(omnuh2, H_0),epsrel=1.49e-09)[0]
    # Utilities to produce a constat DLSS curve

    H_array = np.linspace(H_0 - H_0 * 0.01, H_0 + H_0 * 0.01, 25)
    omnuh2_array = np.zeros_like(H_array)
    for j, h0 in enumerate(H_array):
        func = lambda omnuh2: (R - quad(integrand, 0, 1100, args=(omnuh2, h0), epsrel=1.49e-11)[0])*100
        omnuh2_array[j] = fsolve(func, fid['omnuh2'])
    ...
    a_fisher = (sigmax_squared + sigmay_squared) / 2. + \
        np.sqrt((sigmax_squared - sigmay_squared) ** 2 / 4. + sigmaxy ** 2)
    b_fisher = (sigmax_squared + sigmay_squared) / 2. - \
        np.sqrt((sigmax_squared - sigmay_squared) ** 2 / 4. + sigmaxy ** 2)
    ...
    if sigmax_squared > sigmay_squared:
        ...
        one_sigma = Ellipse((fid[parameters[0]], fid[parameters[1]]), 2. * np.sqrt(np.amax([a_fisher, b_fisher])) * 1.52, 2. * np.sqrt(np.amin([a_fisher, b_fisher])) * 1.52,
                            angle=angle_fisher, linewidth=0.8, fill=True, alpha=0.4,color = colors[i])

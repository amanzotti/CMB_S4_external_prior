...
for i, key in enumerate(par_gaps.keys()):
    ...
    plot3 = plt.loglog(dats2[:, 0, 0], np.abs(
        np.nan_to_num(-dats2[:, 1, 4 * i + 4] + 8. * dats2[:, 1, 4 * i + 3] - 8. * dats2[:, 1, 4 * i + 2] + dats2[:, 1, 4 * i + 1]) / (12 * par_gaps2[key])/dats2[:, 1, 0]), linewidth=1 ,label = 'run4' )
...
for i, key in enumerate(par_gaps.keys()):
    ...
    plot3 = plt.loglog(dats[:, 0, 0], np.abs(
        np.nan_to_num( -dats[:, 2, 4 * i + 4] + 8. * dats[:, 2, 4 * i + 3] - 8. * dats[:,2 , 4 * i + 2] + dats[:, 2, 4 * i + 1]) / (12 * par_gaps[key])/dats[:, 2, 0]) , linewidth=1)

    plot3 = plt.loglog(dats2[:, 0, 0], np.abs(
        np.nan_to_num(-dats2[:, 2, 4 * i + 4] + 8. * dats2[:, 2, 4 * i + 3] - 8. * dats2[:, 2, 4 * i + 2] + dats2[:, 2, 4 * i + 1]) / (12 * par_gaps2[key])/dats2[:, 2, 0]), linewidth=1 ,label = 'run4' )
...
for i, key in enumerate(par_gaps.keys()):
    ...
    plot3 = plt.loglog(dats[:, 0, 0], np.abs(
        np.nan_to_num(-dats[:, 5, 4 * i + 4] + 8. * dats[:, 5, 4 * i + 3] - 8. * dats[:, 5, 4 * i + 2] + dats[:, 5, 4 * i + 1]) / (12 * par_gaps[key])/dats[:, 5, 0]), linewidth=1)

    plot3 = plt.loglog(dats2[:, 0, 0], np.abs(
        np.nan_to_num(-dats2[:, 5, 4 * i + 4] + 8. * dats2[:, 5, 4 * i + 3] - 8. * dats2[:, 5, 4 * i + 2] + dats2[:, 5, 4 * i + 1]) / (12 * par_gaps2[key])/dats2[:, 5, 0]), linewidth=1 ,label = 'run4' )
...
for i, key in enumerate(par_gaps.keys()):
    ...
    plot3 = plt.loglog(dats[:, 0, 0], np.abs(
        np.nan_to_num(-dats[:, 6, 4 * i + 4] + 8. * dats[:, 6, 4 * i + 3] - 8. * dats[:, 6, 4 * i + 2] + dats[:, 6, 4 * i + 1]) / (12 * par_gaps[key])/dats[:, 6, 0]), linewidth=1)


    plot3 = plt.loglog(dats2[:, 0, 0], np.abs(
        np.nan_to_num(-dats2[:, 6, 4 * i + 4] + 8. * dats2[:, 6, 4 * i + 3] - 8. * dats2[:, 6, 4 * i + 2] + dats2[:, 6, 4 * i + 1]) / (12 * par_gaps2[key])/dats2[:, 6, 0]), linewidth=1 ,label = 'run4' )

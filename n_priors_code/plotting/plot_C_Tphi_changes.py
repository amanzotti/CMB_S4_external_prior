...
for i, key in enumerate(par_gaps.keys()):
    ...
    for x in xrange(0,4):
        ...
        plot2 = plt.semilogx(dats[:, 0, 0], (dats[:, 6, 4 * i + x+1]-dats[:, 6, 0])/dats[:, 6,0], linewidth=1,rasterized=True,label=r'${} = {}$'.format(label[key],values[key][x]))
    ax1.set_ylim((-4,4))
    ...
    plt.savefig('../data/{}/run{}/output/CL_Tphi_{}.pdf'.format(data_type,run_idx,str(key)), dpi=400, papertype='Letter',

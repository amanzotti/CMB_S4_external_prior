...
for i, key in enumerate(par_gaps.keys()):
    ...
    for x in xrange(0,4):
        ...
        plot2 = plt.semilogx(dats[:, 0, 0], (dats[:, 5, 4 * i + x+1]-dats[:, 5, 0])/dats[:, 5,0], linewidth=1,rasterized=True,label=r'${} = {}$'.format(label[key],values[key][x]))
    ...
    plt.savefig('../data/{}/run{}/output/CL_phi_{}.pdf'.format(data_type,run_idx,str(key)), dpi=400, papertype='Letter',

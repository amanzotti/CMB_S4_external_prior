...
def C(iell, ell, parbin, data):
    ...
    N = 0.*(s * np.pi / 180. / 60.) ** 2 * math.exp(ell * (ell + 1.) * t ** 2 / 8. / math.log(2))
    ...
    # TE,EE,Ephi
    # phiT,phiE,phiphi
    C = np.array([data[iell, 1, parbin] / fac + N])
...
np.savetxt('output_TT/no_marginalized_ell.txt',np.column_stack((np.arange(lmin, lmax),no_marginalized_ell)))
np.savetxt('output_TT/marginalized_ell.txt',np.column_stack((np.arange(lmin, lmax),marginalized_ell)))
...
np.savetxt('output_TT/sigma_H0_1percent.txt', d2)
np.savetxt('output_TT/sigma_H0_noPrior.txt', d)
np.savetxt('output_TT/sigma_H0_perfect_prior.txt', d3)
...
np.savetxt('output_TT/sigma_tau_1percent.txt', d2)
np.savetxt('output_TT/sigma_tau_noPrior.txt', d)
np.savetxt('output_TT/sigma_tau_perfect_prior.txt', d3)
...
np.savetxt('output_TT/sigma_ns_1percent.txt', d2)
np.savetxt('output_TT/sigma_ns_noPrior.txt', d)
np.savetxt('output_TT/sigma_ns_perfect_prior.txt', d3)
...
# print param_cov
np.savetxt('output_TT/param_cov.txt', param_cov)
np.savetxt('output_TT/invetered_sqrt_fisher.txt', np.sqrt(fisher_inv))

...
As current BAO data (labeled BAO15), we follow \cite{allison:2015} and we take into account 6dFGRS \cite{beutler:2011}, SDSS MGS \cite{ross:2015} together with LOW-Z and C-MASS \cite{anderson:2014}.
...
The horizonal line correspond to CMB S4 experiment + Planck Pol + DESI.
...
The position of the CMB peaks, well constrained by measurement, in a flat universe is a function $\Omega_ch^{2.93}$ \cite{planck-collaboration:2014} 
. For this reason an external prior on $H_{0}$ will improve the internal reconstruction of $\Omega_ch^2$, leading back to tighter neutrino mass constraints.
...
\caption{Same as \reffig{prior_omeganuh2} but with $\ell_{\rm min}>50$. Here $A_{s}$ and $\tau$ priors will also helps because of their degeneracy with $\sum m_{\nu}$.}
...
a measurement of the optical depth is required to infer $A_{s}$ and therefore to tighten the constraints on $\sum m_\nu$. If CMB S4 will not be designed to measure scales $\ell<50$ an external prior on the optical depth from future 21 cm surveys \cite{liu:2015} or CMB satellite experiments like PIXIE \cite{kogut:2011} or balloon like SPIDER \cite{filippini:2010}, will be needed to fully exploit the CMB constraining power.

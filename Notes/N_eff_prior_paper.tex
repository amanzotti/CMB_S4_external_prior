...
As it happened in the past, this new sensitivity will improve our understanding of several areas of astrophysics like dark matter, inflation, Dark Energy and neutrinos. 
...
Because relativistic species, like neutrinos, present in the early Universe are the main drivers of the cosmic expansion their number is encapsulated in the expansion rate $H(z)$. This rate can be powerfully tested using the CMB. Mainly by carefully comparing the sound horizon scale, obtained from the CMB peaks positions, and the Silk damping scale (see \cite{2013arXiv1309.5383A}, \cite{2013PhRvD..87h3008H} and references therein). 
The future experiments, with their high resolution, will be able to push their measurements deeply into the damping tale and will unequivocally test with data the predicted value of the standard model $N_{\rm eff}=3.046$.
The total mass of neutrinos, on the other hand, has a modest effect on the CMB because, for the range of masses allowed by recent constraints ($M_{\nu}<230$ meV from \cite{2014A&A...571A..16P}), neutrinos are still relativistic at the last scattering surface. As other low redshift effects however, massive neutrinos modify the CMB by altering the growth of the large scale structure responsible for the the lensing of the CMB photons. Different neutrino masses consequently lead to different CMB lensing. Stage IV experiment will measure small scales temperature and polarization anisotropies with low noise drastically improving the lensing reconstruction. This will turn in a  precise constraints of the sum of neutrino masses with a possible hint of the hierarchies of the individual masses. 
To put this progress in prospective, CMB alone will constrain the relativistic degree of freedom $N_{\rm eff}$ with a $1\%$ precision and the total mass of neutrinos at $60\%$ with a factor of two improvement on $M_{\nu}$ when we introduce priors from baryon acoustic oscillations (BAO) data \cite{wu:2014}. The future generation of CMB experiments will be a big step in the understanding of the neutrino sector.

As previously mentioned, CMB is also sensitive to the properties of Dark energy (see \cite{2010MNRAS.405.2639J}). The current generation of experiments measured with percent accuracy the energy density of Dark Energy. The challenge for future experiments is to reveal the nature of this mysterious component. For example a crucial step will be to investigate any possible deviations in the equation of state from the value $w=-1$ predicted by a cosmological constant model. Furthermore CMB will also be a powerful probe of any time dependence of the dark energy equation of state.
Indeed dark energy affects the CMB because it alters the universe's expansion consequently changing the distance to the last scattering surface. Furthermore different DE models lead to a different growth of large scale structure that can be inferred from the CMB lensing. However, dark energy properties are strongly degenerate with other geometrical parameters like $H_{0}$ and $\Omega_{k}$. Similarly to the neutrino sector, to get competitive DE constraints, CMB experiments will have to rely on external prior coming from BAO and supernova experiments.
...
To improve the synergy between different experiments and to guide the plan of future experiments, in this work we study in detail the dependance of CMB future constrains on the external priors assumed.

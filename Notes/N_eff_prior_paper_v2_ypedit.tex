...
Each prior is expressed in units of the error that would be obtained internally from the CMB S4 experiment together with the current BAO and Planck results; e.g., $\sigma(H_0)_{\rm CMB+Planck pol+BAO15}=0.5\%$, so the value of unity on the x-axis corresponds to imposing an external prior on $H_0$ of 0.5\%. $H_{0}$ and $\Omega_{c}h^{2}$ are shown to be the most important external priors to improve neutrino mass constraints. We chose a fiducial value $\sum m_{\nu}=85 $ meV. The bottom-most curve shows the impact of imposing the given prior on {\it all} the other parameters simultaneously. The blue horizontal line correspond to the CMB S4+Planck pol+DESI constraint.}
...
Furthermore CMB lensing probes the growth of structure at late times which is different for different dark energy equations of state. \am{we should probably add a few lines on the physical constraint from BAO}
%==========
...
\reffig{prior_w} shows that without any priors, CMB S4 experiments together with the current state of the art BAO measurement (BAO15) will not do much better than current constraints, which hover around 5\%. However, an external constraint on the Hubble constant would improve the CMB constraints on $w$ considerably. 
...
The design of the next CMB Stage IV experiments represents an exciting challenge. The possible reward is remarkable. We can finally put a tight constraint on the total mass of neutrinos with deep implications on particle physics like, for example, the solution of the mass hierarchy problem. We can also measure with great accuracy the number of relativistic species, thereby testing the standard model prediction and possibly probing new physics. The same is true for dark energy: we can tighten the constraint on $w$ and test the nature of the accelerated expansion of the universe.
